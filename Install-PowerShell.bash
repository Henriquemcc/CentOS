...
function InstallPowerShell_On_x86_64
{
...
function InstallPowerShell_On_arm
{
  # Downloading file
...
function InstallPowerShell_On_arm32
{
    InstallPowerShell_On_arm $url_arm32_tar_gz
}

function InstallPowerShell_On_arm64
{
    InstallPowerShell_On_arm $url_arm64_tar_gz
}

# x86_64
if [ "$(uname -m)" == "x86_64" ]; then
  InstallPowerShell_On_x86_64
...
# Arm
    ...
    if [ "$(uname -m)" == "aarch64" ] || [ "$(uname -m)" == "arm64" ] || { [ "$(command -v getconf)" ] && [ "$(getconf LONG_BIT)" == 64 ]; } ; then
      InstallPowerShell_On_arm64
    # 32 bits
    else
      InstallPowerShell_On_arm32
    fi
fi
